...
public class Edit extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String ma = request.getParameter("ma"); // Đổi từ int sang String
        String ten = request.getParameter("ten");
        String tg = request.getParameter("tacgia");
        String nxb = request.getParameter("nxb");

        int status = (int)ss.getAttribute("status");
        if(status == 0) {
            request.getRequestDispatcher("/login.jsp").forward(request, response);
        } else {
            try {
                DBConnect conn = new DBConnect();
                boolean isUpdated = conn.editID(ma, ten, tg, nxb); // Gọi phương thức với ma là String
                if (!isUpdated) {
